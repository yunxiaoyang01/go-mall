...
import (
	...
	"github.com/yunxiaoyang01/go-mall/go-mall-admin/resource/autocode_template/subcontract"

	"github.com/yunxiaoyang01/go-mall/go-mall-admin/global"
	"github.com/yunxiaoyang01/go-mall/go-mall-admin/model/system"
	"github.com/yunxiaoyang01/go-mall/go-mall-admin/utils"
...
func Init(Package string) {
	...
	packageInjectionMap = map[string]astInjectionMeta{
		packageServiceName: {
			...
			importCodeF:  "github.com/yunxiaoyang01/go-mall/go-mall-admin/%s/%s",
			packageNameF: "%s",
...
func (autoCodeService *AutoCodeService) CreateTemp(autoCode system.AutoCodeStruct, ids ...uint) (err error) {
	...
	if autoCode.AutoMoveFile { // 判断是否需要自动转移
		...
		var routePath = filepath.Join(global.GVA_CONFIG.AutoCode.Root,
			global.GVA_CONFIG.AutoCode.Server, global.GVA_CONFIG.AutoCode.SInitialize, "router.go")
		var imporStr = fmt.Sprintf("github.com/yunxiaoyang01/go-mall/go-mall-admin/model/%s", autoCode.Package)
